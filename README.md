...
why espeak-ng and its GPL obligation are gone ([ADR 0012](docs/adr/0012-supertonic-como-motor.md)).

`models/supertonic/contrato.json` is the contract between the Python and the TypeScript side, and
it is **generated by reading the graphs themselves**, so nobody types a shape and nothing can drift.

**Cloning does not work yet.** Supertone never published the encoder that turns a recording into a
style, and their hosted Voice Builder went dark on 2026-08-31. Four attempts at rebuilding it are
measured in [docs/evidencia.md](docs/evidencia.md) — the best reaches 0.227 against a target of
0.55, and the most useful finding is that inverting the vocoder recovers a real recording's latent
with the speaker identity intact (0.832), so the information is there and the missing piece is a
loss function that sees it.

## Run it

**The fastest way: don't install anything.** Open the page and press **«descargar de Hugging
Face»** — the browser pulls the four graphs (398 MB) straight from the archive and speaks. Hugging
Face serves them with the CORS headers that makes this legal for a browser, which is also why the
voice packs live there ([ADR 0011](docs/adr/0011-paquetes-de-voz-y-hardware.md)). Keeping a local
copy (`uv run python -m ttspro.supertonic.descargar`) only makes every later reload faster.
